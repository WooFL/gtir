...
*Takeaway:* import-reachability is a strong correctness signal but only as good as import resolution — gtir's relative-only resolution makes it aggressive on monorepos. **Next lever:** resolve tsconfig `paths` / `package.json` so `@pkg/…` imports become edges, recovering the cross-package legit calls the prior currently can't see. (Investigated a language-agnostic specifier-token version, then **skipped** it — the dropped edges degrade to harmless `ambiguous`, and the fuzzy token-match risked re-noising the tier we'd just cleaned. YAGNI.)

### Finding 6 — incremental refresh silently corrupted cross-file edges (found by a refresh-vs-rebuild test)

A diagnostic — build → mutate one file → incremental refresh → compare against a from-scratch rebuild — caught a real bug. `indexEdges` re-extracted edges only for **changed** files; an edge `a.ts → b.ts#helper` is stored under `from_path: a.ts`, so deleting/moving `b.ts` left `a.ts` (unchanged, same mtime) pointing at a **non-existent** definition. Plus deletions never reached `indexEdges` at all (a delete-only refresh hit a no-chunks early-return before the edge pass). So `callers`/`impact`/`cycles` traversed dead edges after every commit until a full `--rebuild`.

**Fix shipped** (`fix(indexer): re-resolve affected callers on refresh`): `indexEdges` now takes the deleted set, and via a reverse lookup over existing edges re-parses every unchanged caller whose resolution could have changed — its `to_path` points into a changed/deleted file, an ambiguous candidate did, or it calls a name a changed file now declares — plus the no-chunks early-return path re-runs edge resolution for deletions. Re-parse (not re-resolve-from-stored) so the transient `isMethod` for the structural filter is recovered fresh. Reviewer traced add/modify/delete/rename/duplicate-def — no remaining divergence; 4 regression tests pin it.

*Takeaway:* the precision work assumed a correct *base* index — but the incremental path that runs on every commit was quietly diverging from a rebuild. **The diagnostic (incremental == rebuild) is the durable tool**; it found a corruption no amount of edge-precision tuning would have. Imports need no re-resolution (path-arithmetic, consistent); notes-link staleness on rename/delete is the analogous remaining gap.
