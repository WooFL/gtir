...
test("buildTools emits search/read/outline/similar/find/callers/callees/neighbors per index + gtir_status", () => {
  ...
  assert.deepEqual(tools.map((t) => t.name), [
    ...
    "gtir_status", "stale_check", "stale_ack", "stale_sync", "notes_for",
  ]);
  ...
  assert.deepEqual(tools.find((t) => t.name === "gtir_status").inputSchema.properties, {});    // status takes no args
});
...
test("handleRequest: tools/list returns the tool set", async () => {
  ...
  assert.deepEqual(r.result.tools.map((t) => t.name),
    ["context_code", "search_code", "read_code", "outline_code", "similar_code", "find_code",
     "callers_code", "callees_code", "neighbors_code", "impact_code", "orphans_code", "cycles_code", "path_code", "gtir_status", "stale_check", "stale_ack", "stale_sync", "notes_for"]);
});
